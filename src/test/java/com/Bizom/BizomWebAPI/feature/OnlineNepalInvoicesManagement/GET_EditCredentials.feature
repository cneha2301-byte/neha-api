@OnlineNepalInvoicesManagement @EditCredentials @Innovation
#this API have notice in response so for a time being we are not testing this API
Feature: Edit Credentials API Testing for Online Nepal Invoices
  As a system user
  I want to test the edit credentials endpoint for online Nepal invoices
  So that I can ensure proper functionality for editing credentials in online Nepal invoices management
  ...
  Scenario: TC_01 Edit credentials with valid cookies and headers
    When I send the GET request to "onlinenepalinvoices_editcredentials" endpoint
  ...
  Scenario: TC_03 Edit credentials without cookies
    When I set up request specification
      ...
      |accept-language|en-US,en;q=0.9|
      |user-agent|Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36|
    And I send the GET request to "onlinenepalinvoices_editcredentials" endpoint
    Then I should see the response code as "200"
    And verify response time is less than "2500" milliseconds
  ...
  Scenario: TC_09 Edit credentials with invalid endpoint
    When I send the GET request to "onlinenepalinvoices_editcredentials_invalid" endpoint
    Then I should see the response code as "200"
    And verify response time is less than "2500" milliseconds
